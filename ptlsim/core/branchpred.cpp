...
ostream& operator <<(ostream& os, const ReturnAddressStackEntry& e) {
  os << "  " << intstring(e.idx, 4) << ": uuid " << intstring(e.uuid, 16) <<
    ", rip " << (void*)(Waddr)e.rip << endl;
  return os;
}
...
struct ReturnAddressStack: public Queue<ReturnAddressStackEntry, SIZE> {
...
#ifdef DEBUG_RAS
    if (logable(5)) ptl_logfile << "ReturnAddressStack::push(uuid " << uuid <<
         ", rip " << (void*)(Waddr)rip << "):" << endl;
#endif
    if (base_t::full()) {
  ...
  void annulpush(const ReturnAddressStackEntry& old) {
#ifdef DEBUG_RAS
    if (logable(5)) ptl_logfile << "ReturnAddressStack::annulpush(old index " << 
        old.idx << ", uuid " << old.uuid << ", rip " << (void*)(Waddr)old.rip << 
        "):" << endl;
#endif
    ...
    pop(dummy);
#ifdef DEBUG_RAS
    if (logable(5)) ptl_logfile << "  Popped speculative push; e.index = " << 
        e.index() << " vs tail " << base_t::tail << endl;
...
#ifdef DEBUG_RAS
    if (logable(5)) ptl_logfile << "ReturnAddressStack::annulpop(old index " <<
         old.idx << ", uuid " << old.uuid << ", rip " << (void*)(Waddr)old.rip <<
         "):" << endl;
#endif
...
#ifdef DEBUG_RAS
    if (logable(5)) ptl_logfile << "  Pushed speculative pop; old.index = " <<
         old.index() << " vs tail " << base_t::tail << endl;
