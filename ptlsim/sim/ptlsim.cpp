...
static void print_banner(ostream& os) {
  ...
  os << "// " << endl;
  os << "//  Git branch '" << stringify(GITBRANCH) << "' on date " << stringify(GITDATE) <<
    " (HEAD: " << stringify(GITCOMMIT) << ")", endl;
  os << "//  Built " << __DATE__ << " " << __TIME__ << " on " << stringify(BUILDHOST) << 
    " using gcc- " << stringify(__GNUC__) << "." << stringify(__GNUC_MINOR__) << endl;
...
extern "C" uint8_t ptl_simulate() {
	...
	if (!machine->stopped) {
        if(logable(1)) {
			ptl_logfile << "Switching back to qemu rip: " << (void *)contextof(0).get_cs_eip()
            << " exception: " << contextof(0).exception_index <<
	...
	sb << endl << "Stopped after " << sim_cycle << " cycles, " << total_insns_committed << " instructions and " <<
	   seconds << " seconds of sim time (cycle/sec: " << W64(double(sim_cycle) / double(seconds)) << 
       " Hz, insns/sec: " << W64(double(total_insns_committed) / double(seconds)) << ", insns/cyc: " <<
       double(total_insns_committed) / double(sim_cycle), ")" << endl;
