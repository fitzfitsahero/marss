...
int BaseMachine::run(PTLsimConfig& config)
{
    ...
    for (;;) {
        ...
        if unlikely (config.stop_at_insns <= total_insns_committed ||
            ...
            ptl_logfile << "Stopping simulation loop at specified limits (" << 
                sim_cycle << " cycles, " << total_insns_committed << " commits)" << endl;
            exiting = 1;
            break;
        }
        if unlikely (exiting) {
    ...
    if(logable(1))
        ptl_logfile << "Exiting out-of-order core at " << total_insns_committed <<
            " commits, " << total_uops_committed << " uops and " << iterations <<
            " iterations (cycles)" << endl;
